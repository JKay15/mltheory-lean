...
| StrictFormalization | Phase1-ExternalCandidates | 外部候选声明级复刻清单（不直接加依赖） | partial | 2026-02-14 | auto-res/lean-rademacher, YuanheZ/lean-stat-learning-theory（README/lakefile/toolchain） | auto-res/lean-rademacher; YuanheZ/lean-stat-learning-theory | 按声明级清单在本仓先复刻并回归验证；仅当 Lean 版本与 license 稳定后评估 git 依赖接入。 |
...
| StrictFormalization | Phase3-Theorem42-ClosureBridge | 严格版 Theorem42：Stone 结果到二层类闭包桥接 | partial | 2026-02-14 | paper-template Theorem42.StoneRoute/StrictFinal/SmokeFinal；mathlib: Set.SeparatesPoints, ContinuousMap.Subalgebra.SeparatesPoints, Subalgebra.separatesPoints_monotone, StoneWeierstrass | leanprover-community/mathlib4 | 继续下沉：尝试把 `hDenseInAdjoin` 也从更原始可检验条件自动推出，最终把 Theorem42 strict 入口收敛到课件层自然前提。 |
