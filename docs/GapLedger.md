...
| StrictFormalization | Phase1-ExternalCandidates | 外部候选声明级复刻清单（不直接加依赖） | partial | 2026-02-14 | auto-res/lean-rademacher, YuanheZ/lean-stat-learning-theory（README/lakefile/toolchain）, Leorasz/uat_proof（README/step1.lean，GitHub API） | auto-res/lean-rademacher; YuanheZ/lean-stat-learning-theory; Leorasz/uat_proof | 优先复刻可独立引用的声明；`Leorasz/uat_proof` 先做路线参考，不作为直接依赖，待其项目骨架成熟后再评估接入。 |
